...
func add_gold(amount: int):
	gold += amount
	print("💰 Получено ", amount, " золота. Всего: ", gold)
	save_progress()
...
func can_afford_upgrade(upgrade_id: String) -> bool:
	return UPGRADES.has(upgrade_id) and not (upgrade_id in unlocked_upgrades) and soul_shards >= UPGRADES[upgrade_id].cost
...
func load_progress() -> void:
	...
	gold = save.get_value("meta", "gold", 0)
	print("👹 Добро пожаловать! Осколков: ", soul_shards, " Золота: ", gold)
