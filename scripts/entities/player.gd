...
# Ссылка на сферу здоровья
var globe_ui: Node = null

# === ПЕРЕМЕННЫЕ ДВИЖЕНИЯ ===
var target_position: Vector2 = Vector2.ZERO
var is_moving: bool = false
var target_type: String = "none"
...
func _ready():
	add_to_group("player")
	collision_layer = 1
	collision_mask = 2 | 4  # Враги(2) и Стены(4)
	...
	# Поиск и инициализация сферы
	_find_globe_ui()

func _find_globe_ui():
	# Надёжный поиск узла "Globe" в любом месте дерева сцены
	globe_ui = _find_node_by_name_recursive(get_tree().root, "Globe")

	if globe_ui:
		if globe_ui.has_method("set_max_health"):
			globe_ui.set_max_health(max_health)
		if globe_ui.has_method("set_health"):
			globe_ui.set_health(current_health)
		print("✅ HealthGlobe подключен!")
	else:
		print("⚠️ HealthGlobe не найден. Сфера не будет обновляться.")

# 🔥 ИСПРАВЛЕНО: параметр "name" переименован в "target_name", 
# чтобы избежать предупреждения SHADOWED_VARIABLE_BASE_CLASS
...
# 🔥 Обновляет ТОЛЬКО сферу здоровья
func _update_globe_display():
	if globe_ui and globe_ui.has_method("set_health"):
		globe_ui.set_health(current_health)
