...
func _carve_wide_corridor(x1, y1, x2, y2):
	var half_down = (corridor_width - 1) / 2
...
func _fill_tiles():
	# Сначала заливаем ВСЮ карту бордовой стеной
	for x in range(MAP_WIDTH + 4):
		for y in range(MAP_HEIGHT + 4):
			tilemap.set_cell(Vector2i(x, y), 1, Vector2i(0, 0))
	
	# Теперь поверх рисуем пол там, где он должен быть
	for x in range(MAP_WIDTH + 4):
		for y in range(MAP_HEIGHT + 4):
			if floor_grid[x][y]:
				tilemap.set_cell(Vector2i(x, y), 0, Vector2i(0, 0))
	
	# Дополнительно обводим пол стенами (уже не обязательно, но оставим для чёткости границ)
	# Это создаст тонкую рамку вокруг комнат и коридоров, хотя она и так бордовая.
	# Можно убрать, если не нужна двойная стена, но оставим для явной границы.
	for x in range(MAP_WIDTH + 4):
		for y in range(MAP_HEIGHT + 4):
	...
	# Границы карты (крайние стены) уже есть, так как мы залили всё стеной, но можно оставить для надёжности
