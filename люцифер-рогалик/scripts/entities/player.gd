...
func _physics_process(_delta: float):
	if is_moving_to_target:
		...
		if distance > 5:  # уменьшил порог для более точной остановки
	...
	# Включаем скольжение вдоль стен с большим числом попыток (max_slides = 6)
	move_and_slide()
	
	# Если мы упёрлись в стену и не двигаемся к цели, добавим перпендикулярное ускорение
	if is_moving_to_target and get_slide_collision_count() > 0:
		# Получаем нормаль первого столкновения
		var collision = get_slide_collision(0)
		var normal = collision.get_normal()
		# Проекция желаемой скорости на направление вдоль стены
		var desired_dir = (target_position - global_position).normalized()
		var wall_tangent = Vector2(-normal.y, normal.x)
		var dot = desired_dir.dot(wall_tangent)
		# Если целевая точка находится вдоль стены, применяем дополнительное ускорение
		if abs(dot) > 0.1:
			# Увеличиваем скорость в этом направлении (но не больше обычной скорости)
			velocity += wall_tangent * dot * speed * 0.5
